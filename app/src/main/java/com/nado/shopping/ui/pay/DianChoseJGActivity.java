...
import com.nado.shopping.bean.DianCompany;
...
import com.nado.shopping.ui.main.ChooseCityActivity;
...
public class DianChoseJGActivity extends BaseActivity {
    ...
    private RecyclerCommonAdapter<DianCompany> myAdapter;
    private List<DianCompany> mBeanList = new ArrayList<>();
...
//        tflActivityParkLot = (TwinklingRefreshLayout) findViewById(R.id.tfl_activity_park_lot);
    ...
    public void initData() {
        ...
        initCompanyList(HomepageConstant.mLocationCity.replace("市",""),HomepageConstant.mLocationProvince.replace("省",""));

    }

    private void initCompanyList(String city,String province) {
        Map<String, String> map = new HashMap<>();
        map.put("province",province);
        map.put("city",city);
        map.put("type","2");
        RequestManager.mRetrofitManager3.createRequest(RetrofitRequestInterface.class).getDianCompany(RequestManager.encryptParams(map)).enqueue(new RetrofitCallBack() {
                ...
                try {
                    ...
                    if (code == 0) {
                        ...
                        for (int i = 0; i <jsonArray.length() ; i++) {
                            ...
                            DianCompany bean=new DianCompany();
                            bean.id=jsonObject.optString("productid");
    ...
    private void showRecycleView() {
        if (myAdapter == null) {
            ...
            myAdapter = new RecyclerCommonAdapter<DianCompany>(mActivity, R.layout.item_jg, mBeanList) {

                @Override
                protected void convert(ViewHolder holder, final DianCompany carChoiceBean, int position) {
    ...
    private void bindCityCompany(DianCompany carChoiceBean) {
    ...
    public void initEvent() {
        tvLayoutTopBackBarEnd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivityForResult(new Intent(v.getContext(), ChooseCityActivity.class).putExtra("url","index.php?g=app&m=power&a=gainprovince"),100);
            }
        });
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==100){
            if(resultCode== Activity.RESULT_OK){
                ...
                initCompanyList(data.getStringExtra("city"),data.getStringExtra("province"));
            }
        }
    }
}
